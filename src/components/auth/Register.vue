<template>
  <div :dir="isRTL ? 'rtl' : 'ltr'"
    ...
    <div class="absolute top-4 right-4 flex gap-2">
      ...
      <button @click="switchLang('ar')" :class="btnClass('ar')"
        class="p-2 rounded-lg bg-white shadow-md hover:shadow-lg transition-all duration-200"
        :title="$t('auth.register.title')">
    ...
    <div class="w-full max-w-md">
      <div class="bg-white rounded-2xl shadow-2xl p-8">
        ...
        <div class="text-center mb-8">
          ...
          <h1 class="text-2xl font-bold theme-text-primary mb-2">{{ $t('auth.register.title') }}</h1>
          <p class="theme-text-secondary">{{ $t('auth.register.subtitle') }}</p>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div>
            ...
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <svg class="h-5 w-5 theme-caption" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498A1 1 0 0121 16.72V20a2 2 0 01-2 2h-1C9.163 22 2 14.837 2 6V5a2 2 0 011-1.732z" />
                </svg>
              </div>
              <input v-model="phone" type="tel" autocomplete="tel" :placeholder="$t('auth.register.phone')"
                class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors"
                :class="{ 'border-red-500': phoneError }" required />
            </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input v-model="email" type="email" autocomplete="email" :placeholder="$t('auth.register.email')"
                class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors"
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input v-model="confirmPassword" :type="showConfirmPassword ? 'text' : 'password'"
                autocomplete="new-password" :placeholder="$t('auth.register.confirmPassword')"
                class="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors"
                :class="{ 'border-red-500': confirmPasswordError }" required />
              <button type="button" @click="showConfirmPassword = !showConfirmPassword"
                class="absolute inset-y-0 right-0 pr-3 flex items-center">
                <svg v-if="showConfirmPassword" class="h-5 w-5 theme-caption" fill="none" stroke="currentColor"
                  viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L21 21">
                  </path>
          ...
          <div class="flex items-center">
            ...
            <label class="ml-2 text-sm theme-text-secondary">
          ...
          <button type="submit" :disabled="loading"
            class="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium theme-text-light bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
        ...
        <div class="mt-6 text-center">
          <p class="text-sm theme-text-secondary">
            ...
            <a href="#" @click.prevent="$emit('switch-auth', 'login')"
  ...
  methods: {
    async submit() {
      ...
      if (!this.validateForm()) {
        // Show first validation error via toast for better visibility
        const firstError = this.firstNameError || this.lastNameError || this.emailError || this.phoneError || this.passwordError || this.confirmPasswordError || this.error
        if (firstError && window.$toast) window.$toast(firstError, 'error')
        return
      }
