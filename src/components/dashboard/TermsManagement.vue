<template>
  <div class="space-y-6" :class="{ 'direction-rtl': isRTL }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
      <div>
        <h1 class="text-2xl font-bold theme-text-primary">إدارة البنود (البنود الرئيسية والفرعية)</h1>
        <p class="text-sm theme-text-muted mt-1">إضافة وتعديل البنود الرئيسية والفرعية المستخدمة في المصروفات والعهد</p>
      </div>

      <button 
        @click="openAddCategoryModal"
        class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl font-medium shadow-md shadow-emerald-500/20 hover:from-emerald-700 hover:to-teal-700 transition-all duration-200"
      >
