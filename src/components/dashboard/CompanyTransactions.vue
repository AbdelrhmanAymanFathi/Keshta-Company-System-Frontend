<template>
  <div class="bg-white rounded-lg shadow p-4">
    <div :class="['flex items-center justify-between mb-4', isRTL ? 'flex-row-reverse' : '']">
      <h3 :class="['text-lg font-medium theme-text-primary flex items-center gap-2', isRTL ? 'text-end' : 'text-start']">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
      <div :class="['bg-white rounded-lg shadow p-4 border-l-4 theme-border-accent', isRTL ? 'text-end' : 'text-start']">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium theme-text-secondary">{{ $t('finance.currentBalance') }}</p>
            <p class="text-2xl font-bold theme-text mt-1">{{ formatCurrency(summary.balance) }}</p>
          </div>
          <svg class="w-10 h-10 theme-text-light/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div :class="['bg-white rounded-lg shadow p-4 border-l-4 border-green-600', isRTL ? 'text-end' : 'text-start']">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium theme-text-secondary">{{ $t('finance.last30dIn') }}</p>
      ...
      <div :class="['bg-white rounded-lg shadow p-4 border-l-4 border-red-600', isRTL ? 'text-end' : 'text-start']">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium theme-text-secondary">{{ $t('finance.last30dOut') }}</p>
    ...
    <div v-if="loading" class="py-10 flex items-center justify-center">
      <svg class="animate-spin -ml-1 mr-3 h-6 w-6 theme-text" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div v-else>
      <div v-if="transactions.items.length === 0" class="p-6 text-center text-sm theme-text-secondary">
      ...
      <div v-else class="overflow-hidden rounded-lg border border-gray-100">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th :class="['px-6 py-3 text-xs font-medium theme-text-muted uppercase tracking-wider', isRTL ? 'text-end' : 'text-start']">{{ $t('labels.date') || 'Date' }}</th>
              <th :class="['px-6 py-3 text-xs font-medium theme-text-muted uppercase tracking-wider', isRTL ? 'text-end' : 'text-start']">{{ $t('labels.type') || 'Type' }}</th>
              <th :class="['px-6 py-3 text-xs font-medium theme-text-muted uppercase tracking-wider', isRTL ? 'text-end' : 'text-start']">{{ $t('labels.description') || 'Description' }}</th>
              <th :class="['px-6 py-3 text-xs font-medium theme-text-muted uppercase tracking-wider', isRTL ? 'text-end' : 'text-start']">{{ $t('dashboard.companyWallet') || 'Branch' }}</th>
              <th :class="['px-6 py-3 text-xs font-medium theme-text-muted uppercase tracking-wider', isRTL ? 'text-end' : 'text-start']">{{ $t('labels.amount') || 'Amount' }}</th>
              <th :class="['px-6 py-3 text-xs font-medium theme-text-muted uppercase tracking-wider', isRTL ? 'text-end' : 'text-start']">{{ $t('#') || 'Ref' }}</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="tx in transactions.items" :key="tx.id" class="hover:bg-gray-50">
              <td :class="['px-6 py-4 whitespace-nowrap text-sm theme-text-primary', isRTL ? 'text-end' : 'text-start']">{{ formatDate(tx.date) }}</td>
              ...
              <td :class="['px-6 py-4 text-sm theme-text-primary', isRTL ? 'text-end' : 'text-start']">{{ tx.description || '-' }}</td>
              <td :class="['px-6 py-4 whitespace-nowrap text-sm theme-text-secondary', isRTL ? 'text-end' : 'text-start']">{{ getBranchName(tx.branchId) }}</td>
              ...
              <td :class="['px-6 py-4 text-sm theme-text-muted', isRTL ? 'text-end' : 'text-start']">{{ getReferenceLabel(tx) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
