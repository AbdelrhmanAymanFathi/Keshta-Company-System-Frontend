      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-indigo-50">
          <tr>
            ...
            <th class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('extracts.phone') }}</th>
            <th class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('extracts.bankName') }}</th>
            <th class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('extracts.accountNumber') }}</th>
            <th class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('extracts.notes') }}</th>
          ...
          >
            ...
            <td class="px-6 py-4">
              <div class="flex gap-3" :class="isRTL ? 'justify-start' : 'justify-end'">
                <button @click.stop="openStatement(c)" class="text-purple-600 hover:text-purple-800" :title="$t('extracts.statement')">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </button>
                <button @click.stop="openWallet(c)" class="text-blue-600 hover:text-blue-800" :title="$t('extracts.wallet')">
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-else>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div class="p-4 bg-gray-50 rounded">
              <div class="text-sm text-gray-500">{{ $t('extracts.balance') || 'Balance' }}</div>
              <div class="text-2xl font-semibold text-red-600">{{ wallet ? wallet.balance : '-' }}</div>
            </div>
            <div class="p-4 bg-gray-50 rounded">
              <div class="text-sm text-gray-500">{{ $t('extracts.totalDeposits') || 'Total Deposits' }}</div>
              <div class="text-lg font-semibold text-green-600">{{ wallet ? wallet.totalDeposits : '-' }}</div>
            </div>
            <div class="p-4 bg-gray-50 rounded">
              <div class="text-sm text-gray-500">{{ $t('extracts.sources') || 'Sources' }}</div>
              <div class="text-sm">
