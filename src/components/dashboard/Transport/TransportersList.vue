<template>
  <div :dir="isRTL ? 'rtl' : 'ltr'" class="p-0 sm:p-0.5 md:p-1 lg:p-0 space-y-6">
    ...
    <div class="app-page-header flex items-center rounded-2xl theme-page-header-bar p-3 sm:p-5 shadow-lg shadow-slate-200/50" :class="isRTL ? 'justify-between' : 'justify-between'">
      <h2 class="text-2xl font-semibold theme-text-primary">{{ $t('transporters.title') }}</h2>
    ...
    <!-- Search -->
    <div class="max-w-md">
      <input v-model="q" @input="onSearchInput" type="search" :placeholder="$t('transporters.searchPlaceholder')"
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="theme-table-thead-gradient">
          <tr>
            ...
            <th class="px-3 py-2 sm:px-6 sm:py-3 text-xs font-medium theme-text-muted uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('transporters.name') }}</th>
            <th class="px-3 py-2 sm:px-6 sm:py-3 text-xs font-medium theme-text-muted uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('transporters.phone') }}</th>
            <th class="px-3 py-2 sm:px-6 sm:py-3 text-xs font-medium theme-text-muted uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('transporters.bankName') }}</th>
            <th class="px-3 py-2 sm:px-6 sm:py-3 text-xs font-medium theme-text-muted uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('transporters.accountNumber') }}</th>
            <th class="px-3 py-2 sm:px-6 sm:py-3 text-xs font-medium theme-text-muted uppercase tracking-wider whitespace-nowrap" :class="textAlign">{{ $t('transporters.notes') }}</th>
          ...
          >
            ...
            <td class="px-6 py-4">
              <div class="flex gap-3" :class="isRTL ? 'justify-start' : 'justify-end'">
                <button @click.stop="openStatement(c)" class="rounded-lg theme-icon-button p-2" :title="$t('transporters.statement')">
        ...
        <div class="flex justify-between items-start" :class="isRTL ? 'flex-row-reverse' : ''">
          <div :class="isRTL ? 'text-right' : 'text-left'">
            ...
            <div class="text-sm theme-text-muted">
              {{ c.phone || '-' }}<br>
              <span v-if="c.bankName">{{ $t('transporters.bankName') }}: {{ c.bankName }}</span><br>
              <span v-if="c.accountNumber">{{ $t('transporters.accountNumber') }}: {{ c.accountNumber }}</span><br>
              <span v-if="c.notes">{{ $t('transporters.notes') }}: {{ c.notes }}</span>
            </div>
          </div>
          <div class="flex flex-col gap-3">
            <button @click.stop="openStatement(c)" class="theme-text text-xs">{{ $t('transporters.statement') }}</button>
            <button @click.stop="openWallet(c)" class="theme-text text-xs">{{ $t('transporters.wallet') }}</button>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-sm sm:max-w-lg lg:max-w-3xl p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <label class="sm:col-span-2 lg:col-span-3">
            ...
            <input v-model="form.name" :placeholder="$t('transporters.placeholders.name')" class="w-full px-3 py-2 border rounded theme-input-focus" />
          </label>
          <label>
            <div class="text-sm mb-1">{{ $t('transporters.phone') }}</div>
            <input v-model="form.phone" :placeholder="$t('transporters.placeholders.phone')" class="w-full px-3 py-2 border rounded theme-input-focus" />
          </label>
          <label>
            <div class="text-sm mb-1">{{ $t('transporters.bankName') }}</div>
            <input v-model="form.bankName" :placeholder="$t('transporters.placeholders.bankName')" class="w-full px-3 py-2 border rounded theme-input-focus" />
          </label>
          <label>
      ...
      <div class="bg-white rounded-lg shadow-lg w-full max-w-3xl p-6 z-10">
        <div class="flex items-start justify-between mb-4">
          <h3 class="text-lg font-semibold" :class="isRTL ? 'text-right' : ''">
            {{ selectedContractor ? selectedContractor.name : $t('transporters.wallet') }}
          </h3>
    ...
    <div v-if="deleteConfirm.open" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/20 backdrop-blur-sm" style="margin-top: 0 !important;">
      <div class="bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-sm p-6 z-10">
        <p class="mb-4 text-center" :class="isRTL ? 'text-right' : 'text-left'">
          {{ $t('transporters.deleteConfirm') }} "<strong>{{ deleteConfirm.item.name }}</strong>"?
        </p>
...
export default {
  ...
  methods: {
    ...
    async saveContractor() {
      ...
      if (!name) {
        if (window.$toast) window.$toast(this.$t('transporters.validationName') || 'Please enter a name', 'warning')
        return
      }
      const payload = { name }
      ...
      try {
        if (this.editing && this.form.id) {
          ...
          if (window.$toast) window.$toast(this.$t('transporters.updateSuccess') || 'Supplier updated successfully', 'success')
        } else {
          ...
          if (created.length > 1) {
            if (window.$toast) window.$toast(this.$t('transporters.addMultipleSuccess') || 'Created separate contractor records for Export and Transport', 'info')
          } else {
            if (window.$toast) window.$toast(this.$t('transporters.addSuccess') || 'Supplier added successfully', 'success')
          }
        }
        this.closeModal()
        ...
        const errorMsg = e.response?.status === 404 
          ? (this.$t('transporters.updateError404') || 'Supplier not found or update endpoint not available')
          : (e.response?.data?.message || this.$t('transporters.addError') || 'Error saving supplier')
    ...
    async doDelete() {
      try {
        ...
        if ((res && res.status === 204) || (data && data.deletedAt)) {
          if (window.$toast) window.$toast(this.$t('transporters.deleteSuccess') || 'Transporter deleted', 'success')
        } else if (data && !data.deletedAt && params.mode) {
          if (window.$toast) window.$toast(this.$t('transporters.availabilityRemoved') || `Availability removed for ${params.mode}`, 'success')
        } else {
          if (window.$toast) window.$toast(this.$t('transporters.deleteSuccess') || 'Transporter deleted', 'success')
        }
        await this.loadContractors()
        ...
        if (window.$toast) window.$toast(this.$t('transporters.deleteError') || 'Error deleting supplier', 'error')
      }
    },
    confirmDelete(c) {
    ...
    async doDeposit() {
      ...
      if (!amount || amount <= 0) {
        if (window.$toast) window.$toast(this.$t('transporters.validationAmount') || 'Enter a valid amount', 'warning')
        return
      }
      try {
        const payload = {
        ...
        if (window.$toast) window.$toast(this.$t('transporters.depositSuccess') || 'Deposit successful', 'success')
        this.depositForm = { amount: '', description: '', date: '' }
      } catch (err) {
        console.error('Error depositing', err)
        if (window.$toast) window.$toast(this.$t('transporters.depositError') || 'Error making deposit', 'error')
      }
    },
    onFileChange(e) {
    ...
    async parseExcelJson(json) {
      ...
      if (window.$toast) window.$toast(this.$t('transporters.imported', { count: added }) || `${added} suppliers imported`, 'success')
    },
    openStatement(c) {
    ...
    goToDetail(c) {
      if (!c || !c.id) return
      this.$router.push({ name: 'contractor-detail', params: { id: c.id }, query: { from: 'transport' } }).catch(() => {})
    }
  }
}
</script>
