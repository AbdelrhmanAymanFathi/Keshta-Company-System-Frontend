<template>
  <div :dir="isRTL ? 'rtl' : 'ltr'" :class="isRTL ? 'direction-rtl p-6' : 'p-6'">
    ...
    <!-- Search -->
    <div class="mb-4">
      <input v-model="q" @input="onSearchInput" type="search" :placeholder="$t('crushers.searchPlaceholder')"
                  ...
                  <div class="flex-shrink-0 h-10 w-10" :class="isRTL ? 'ml-4 order-2' : 'mr-4 order-1'">
                    <div
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-6 w-6 text-white">
                        <path d="M2 12h20l-2-4h-6l-2-3h-4l-2 3H4z"></path>
        ...
        <div>
          <div class="flex items-center mb-2" :class="isRTL ? 'flex-row-reverse' : 'flex-row'">
            <div class="flex-shrink-0 h-8 w-8" :class="isRTL ? 'ml-3 mr-0' : 'mr-3'">
              <div
                class="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                  stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 text-white">
                  <path d="M2 12h20l-2-4h-6l-2-3h-4l-2 3H4z"></path>
                  <circle cx="8" cy="17" r="3"></circle>
                  <circle cx="16" cy="17" r="3"></circle>
                </svg>
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 z-10 mx-4">
        ...
        <div class="mt-6 flex gap-3" :class="isRTL ? 'justify-start flex-row-reverse' : 'justify-end'">
          ...
          <button @click="saveCrusher" :disabled="loading" :class="['px-4 py-2 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors',
            ...
            <span v-if="loading" class="flex items-center" :class="isRTL ? 'flex-row-reverse' : ''">
              <svg class="animate-spin h-4 w-4 text-white" :class="isRTL ? 'ml-2 -mr-1' : '-ml-1 mr-2'"
                viewBox="0 0 24 24">
    ...
    <div v-if="deleteConfirm.open" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 z-10 mx-4">
        ...
        <div class="flex gap-3" :class="isRTL ? 'justify-start flex-row-reverse' : 'justify-end'">
          ...
          <button @click="doDelete" :disabled="loading" :class="['px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors',
            loading ? 'opacity-50 cursor-not-allowed' : '']">
            <span v-if="loading" class="flex items-center" :class="isRTL ? 'flex-row-reverse' : ''">
              <svg class="animate-spin h-4 w-4 text-white" :class="isRTL ? 'ml-2 -mr-1' : '-ml-1 mr-2'"
                viewBox="0 0 24 24">
  ...
  methods: {
    async loadCrushers() {
      try {
        ...
        this.crushers = Array.isArray(res.data.items) ? res.data.items : (Array.isArray(res.data) ? res.data : [])
        this.total = res.data.total || this.crushers.length
